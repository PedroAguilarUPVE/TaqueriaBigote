...
public class PanelOrden extends JPanel {
	...
	private void crearZonaProductos() {
		...
		panelGridProductos = new JPanel(new FlowLayout(FlowLayout.LEFT, 18, 18));
		panelGridProductos.setBackground(fondoClaro);

		JScrollPane scroll = new JScrollPane(panelGridProductos, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
	...
	private void crearResumen() {
		...
		modeloResumen = new DefaultTableModel(new Object[] { "ID", "Producto", "Cant.", "Subtotal", "PrecioUnitario" },
				0) {
			@Override
	...
	public void cargarProductos() {
		...
		if (filtroActual.equals("Todos"))
			lista = CVentas.cargarProductos();
		else
			lista = CVentas.cargarProductosPorCategoria(filtroActual);
	...
	private void mostrarProductos(List<MProductos> lista) {
		...
		for (MProductos p : lista) {
			if (!p.isActivo())
				continue;
			if (p.getNombre().equalsIgnoreCase("Menudo") && !esFinDeSemana)
				continue;
			if (!filtroActual.equals("Todos") && !p.getCategoria().equalsIgnoreCase(filtroActual))
				continue;
	...
	private JPanel crearTarjetaProducto(MProductos p) {
		...
		card.setPreferredSize(new Dimension(200, 310));
	...
	private void completarOrden() {
		...
		try {
			...
			// Preguntar cuánto paga el cliente
			String pagoStr = JOptionPane.showInputDialog(this, "TOTAL: $" + total + "\n¿Con cuánto paga?");

			if (pagoStr == null) return; // canceló

			try {
			    double pago = Double.parseDouble(pagoStr);

			    if (pago < total) {
			        JOptionPane.showMessageDialog(this, "El pago es insuficiente.", "Error", JOptionPane.ERROR_MESSAGE);
			        return;
			    }
	...
	private void imprimirTicket(int idPedido) {
		try {
			...
			if (print.getPages() == null || print.getPages().isEmpty()) {
				JOptionPane.showMessageDialog(null, "No hay datos para imprimir", "Aviso",
						JOptionPane.INFORMATION_MESSAGE);
				return;
			}
